...
def load_dataset(cfg, split, tokenizer=None):
    dataset_name = cfg['data']['name']
    if tokenizer is None:
        raise ValueError("Tokenizer must be provided to load_dataset.")
    if dataset_name == 'lener':
        from .lener.lener_dataset import get_raw_split
        raw_split = get_raw_split(split)
        tokenized_aligned_dataset = raw_split.map(tokenize_and_align_labels, batched=True, fn_kwargs={"tokenizer": tokenizer, "max_seq_length": cfg['data']['max_seq_length']})
        torch_ds = tokenized_aligned_dataset.with_format("torch")
    else:
        raise NotImplementedError(f"Dataset {dataset_name} not implemented in load_dataset.")
    
    SELECT = cfg['data']['select']
    if SELECT is None or (SELECT <= -1):
        print("No selection limit applied, using full dataset.")
    else:
        print(f"Selection limit applied, loading {SELECT} instances of data from the {split} split.")
        torch_ds = torch_ds.select(range(SELECT))
        return torch_ds
